...
import { AlertDialog } from '@inan/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      inanper_input.value === '' ||
      Number(inanper_input.value) === 0 ||
      !Number(inanper_input.value) ||
      isNaN(Number(inanper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const inanper = inan_to_mojo(inanper_input.value);
    const inanper_value = Number.parseInt(Number(inanper));
    const userpubkey = userpubkey_input.value;
    const amount = inan_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = inan_to_mojo(fee_input.value);
