...
const getInanVersion = () => {
  let version = null;
  const exePath = getExecutablePath('Inan');
  // first see if we can get a Inan exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or Inan is on the path
    try {
      version = child_process
